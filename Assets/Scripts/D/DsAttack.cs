...
public class DsAttack : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetButtonDown("Jump") && DsMove.stunned == false && Bullets < 5 && !OnReload && !Ulting)
        {
            OnReload = true;
            StartCoroutine("Reroading");
        }

        if (Input.GetButtonDown("Fire2") && DsMove.stunned == false && !Ulting && !OnReload && onUlt)
        {
            onUlt = false;
            Ulting = true;
            Bullets = 5;
        ...
        if (Input.GetButtonDown("Fire1") && DsMove.stunned == false && Time.time > nextfire && !Ulting && Bullets >= 1 && !OnReload)
        {
            if (Bullets != 2)
            {
                IsIdle = DsMove.idle;
